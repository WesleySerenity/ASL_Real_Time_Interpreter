...
class_labels = list(string.ascii_uppercase)   # A–Z
dataset_size = 150

cap = cv2.VideoCapture(1)

# ----- desired window / frame size (change if you want) -----
WIN_W, WIN_H = 1280, 720

# Make the window this exact size
...
for label in class_labels:
    ...
    # Preview loop until S is pressed
    while True:
        ret, frame = cap.read()
        ...
        # Resize camera image to match window exactly
        frame_resized = cv2.resize(frame, (WIN_W, WIN_H), interpolation=cv2.INTER_AREA)

        # Text overlay
        cv2.putText(frame_resized, f"Ready for Letter {label}?",
    ...
    while counter < dataset_size:
        ...
        # Save full-res original frame (or change to frame_resized if you prefer)
